...
def _patch_single_slot_runtime(tmp_path: Path, monkeypatch) -> dict[str, list]:
    ...
    def start_consumers(env, evidence, value):
        del evidence, value
        calls["consumer_starts"].append(env["BOT_IMAGE"])
        markers = ("worker_started",)
        return (
...
def test_single_slot_rollout_replaces_legacy_services_and_records_success(
    ...
    assert calls["consumer_starts"] == ["registry/bot@sha256:" + "b" * 64]
    assert (
...
def test_single_slot_failure_after_stop_restores_previous_images(
    ...
    assert calls["consumer_starts"] == ["registry/bot@sha256:" + "a" * 64]
...
def test_single_slot_rollback_requires_verified_consumer_ownership(
    ...
    def fail_consumer_ownership(env, evidence, value):
...
def test_single_slot_stop_is_ordered_and_uses_bounded_timeouts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        deploy,
        "_consumer_lease",
        lambda *args: deploy.ConsumerLease("worker", "worker-id", "now", ("worker_started",)),
    )
...
def test_single_slot_stop_fails_closed_before_backend_when_worker_drain_is_unconfirmed(
    tmp_path: Path, monkeypatch
) -> None:
    config = _config(tmp_path)
    commands = []
