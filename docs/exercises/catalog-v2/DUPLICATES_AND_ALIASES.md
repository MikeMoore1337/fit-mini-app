...
Task 120D реализовал это решение без миграции и удаления строки:
`kettlebell-goblet-squat.canonical_slug = goblet-squat` в API, а оба search consumer
группируют записи до отображения. Существующие ID и ссылки программ остаются валидными.
...
Реализация Task 120D не хранит broad ambiguous terms как одинаковый exact alias разных
canonical records. Multi-token search по title/alias/equipment/tags может вернуть несколько
релевантных компактных строк, но exact normalized alias collision является ошибкой validator.
