...
но не входят в расчёты рабочего объёма, рекомендаций прогрессии, личных рекордов, распределения
RIR и нагрузки на мышцы. Рабочий объём и описательная аналитика учитывают рабочие подходы и
дроп-сеты. Детерминированная рекомендация следующей нагрузки использует только рабочие подходы,
потому что дроп-сет намеренно меняет нагрузку внутри упражнения. Дроп-сеты возвращаются явно как
