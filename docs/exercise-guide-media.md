...
- для 149 упражнений есть две проверенные статические позиции; имена файлов `start` и `active`
  сохраняются как технические, а пользовательские подписи задаются отдельно для каждого
  упражнения;
...
Для динамических силовых упражнений подписи показывают позитивную фазу (движение с усилием) и
негативную фазу (контролируемый возврат). Порядок определяется отдельно для каждого упражнения:
первый файл не считается позитивной или негативной фазой только из-за суффикса `start`. Для
статических, циклических и составных движений используются подходящие подписи подготовки,
удержания, цикла или последовательности, чтобы не приписывать им недостоверную фазу сокращения.
В API динамическая пара упорядочена как позитивная, затем негативная фаза независимо от имён
исходных файлов.
