...
он отделяет увиденную механику продукта от будущего собственного аккаунта. Перед переходом к login
удаляются все demo credentials; после регистрации или входа сразу открывается чистый `/app` без
переноса fixture state. Заполнение профиля остаётся необязательным и выполняется позже по intent.
