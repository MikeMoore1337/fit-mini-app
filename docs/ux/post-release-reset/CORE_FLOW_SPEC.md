...
3. Доступны максимум три действия: `Создать программу` (primary), `Записать питание`,
   `Добавить активность`.
...
1. `Программа` -> `Создать программу`; название имеет безопасный default, frequency optional.
2. Добавить первый тренировочный день; выбрать template пустого/готового дня.
3. Добавить упражнения через picker, проверить summary, `Сохранить программу`.

Расписание, rest presets, RIR defaults, оборудование, reorder, supersets и detailed progression не
удаляются: они находятся в `Настроить подробнее` либо в day/program detail после создания.
Primary save никогда не находится внутри disclosure. Validation остаётся рядом с причиной блока.
