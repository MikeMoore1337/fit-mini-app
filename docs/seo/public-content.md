...
каноническую статью в новой вкладке. В Web база знаний доступна из навигации приложения. В Telegram
Mini App отдельная библиотека и публичный reader не показываются: прямой переход на публичный
маршрут возвращает пользователя в `/app`, а внешняя ссылка остаётся явным переходом на сайт.
