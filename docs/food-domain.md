...
локальную дату, ведёт завтрак, обед, ужин и перекусы, а также видит серверные итоги, цель и остаток
КБЖУ. Будущие даты недоступны. Небольшое превышение ориентира описывается нейтральным текстом без
аварийной визуальной семантики.

Основной сценарий добавления работает только с локальными недавними, избранными, личными и
системными продуктами. Поиск начинается после двух символов с задержкой 250 мс; внешний provider
не вызывается автоматически. Количество можно задать в граммах или стандартных порциях, когда их
масса известна. Черновик выбранного продукта и количества сохраняется локально для аккаунта, даты
и приёма пищи до успешной записи, поэтому восстанавливаемая сетевая ошибка или перезагрузка не
стирает ввод. В черновике остаются только ID, отображаемые макронутриенты, стандартная порция и
введённое количество, а не полный API-объект продукта. Logout, смена аккаунта и успешное удаление
аккаунта очищают черновик централизованно. Изменение и удаление записи пересчитывают день через
существующий diary API.
