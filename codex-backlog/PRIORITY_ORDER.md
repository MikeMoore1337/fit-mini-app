...
production baseline остаётся `DESIGN_V2_1`. Task `114` назначена следующей, но её lifecycle не начат.
...
`Stabilization принята. Можно переходить к Task 114.` получена `2026-08-30`. Это снимает owner gate,
но не запускает Task `114`: для её lifecycle требуется отдельная команда владельца.
...
```text
113 [COMPLETED] -> 113A [COMPLETED, OWNER ACCEPTED] -> 114 [CURRENT, NOT STARTED] -> 115A -> OWNER APPROVAL
...
основную release-последовательность; current/not started task теперь `114`.
...
Task `112` не изменяет текущий UX-reset cycle. Owner gate Task `113A` снят, но Task `114` не
