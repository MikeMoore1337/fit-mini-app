...
- umbrella `46C` и tasks `46C.1-46C.6` - шесть owner-approved remediation change sets;
...
## Статус и порядок запуска

Блок `46A-46J`, включая `46C.6` и owner checkpoints, завершён. Следующая задача —
`47-profile-account-experience.md`. Инструкции ниже сохранены как история порядка выполнения блока.
...
Каждую следующую task запускайте в отдельном чате/сессии. Tasks `46C.1-46C.6` выполняются строго
последовательно и каждая завершается отдельным commit/STOP. Task `46D` начинается только после
завершения всех шести. Tasks `46E`, `46F`, `46G` и `46H` содержат обязательные остановки для выбора,
