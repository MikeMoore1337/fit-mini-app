...
- [ ] task `74A` — следующая после `74` в основной release-последовательности и не начата;
- [ ] owner-selected task `89A` остаётся `PENDING`, но не является текущей; task `90` следует после неё только в отдельном Telegram news потоке;
- [ ] remaining release tasks `74-79` и остальные post-release tasks сохраняют собственные Trigger, dependency и owner decision.

Не выполнять повторно tasks `00-73A`, включая `69B`, и tasks `88-89`. Task `74` только назначена:
её реализация и lifecycle начинаются в отдельной сессии. Tasks `74A`, `89A`, `90` и другие pending
