...
def _stop_legacy_services(config: DeployConfig) -> None:
    _stop_slot_consumers("legacy", config)
    ...
    if not _service_is_running("backend"):
        raise DeploymentError("legacy service backend stopped before maintenance handoff")
    _compose("stop", "-t", str(config.backend_drain_seconds), "backend")
    if _service_is_running("backend"):
        raise DeploymentError("legacy service backend remained running after stop")
